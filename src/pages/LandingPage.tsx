...
import {
  ...
  Send, Calendar, Luggage, RotateCcw, Zap
} from 'lucide-react';

const PHONE = '+33 6 XX XX XX XX';
const EMAIL = 'contact@safir-vtc.fr';
...
const reviews = [
  { name: 'Jean-Pierre M.', rating: 5, text: 'Service impeccable depuis Toulon jusqu\'à Nice. Chauffeur ponctuel, véhicule luxueux. Je recommande vivement Safir VTC pour tous vos transferts.', date: 'Il y a 2 jours' },
  ...
  { name: 'Thomas B.', rating: 5, text: 'Client régulier depuis 1 an pour mes déplacements pro sur la Côte d\'Azur. Safir VTC, c\'est la garantie d\'un service premium à chaque trajet.', date: 'Il y a 1 semaine' },
...
export default function LandingPage() {
  ...
  const estimatePrice = () => {
    ...
    // Mock calculation: simulate based on text length ratio
    const base = Math.round(25 + Math.random() * 180);
    const withType = form.type === 'round_trip' ? Math.round(base * 1.8) : base;
    setPriceEstimate(withType);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          ...
          <div className="max-w-4xl mx-auto">
            <div className="glass rounded-2xl p-6 lg:p-8 glow-sapphire">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <div className="relative">
                    <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-sapphire-400" />
                    <input value={form.departure} onChange={(e) => setForm({ ...form, departure: e.target.value })} type="text" placeholder="Adresse de départ..." className="input-field pl-9" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input value={form.arrival} onChange={(e) => setForm({ ...form, arrival: e.target.value })} type="text" placeholder="Adresse d'arrivée..." className="input-field pl-9" />
                  </div>
                </div>
                <div>
              ...
              <button onClick={estimatePrice} className="w-full py-2 mb-4 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] text-sm text-noir-300 hover:text-white transition-all flex items-center justify-center gap-2">
                <Zap className="w-4 h-4 text-sapphire-400" />
                Estimer le tarif
              </button>

              {priceEstimate !== null && (
                <div className="mb-5 flex items-center justify-between px-5 py-4 rounded-xl bg-sapphire-600/10 border border-sapphire-500/20">
                  <div>
      ...
      <section className="py-20 lg:py-28 border-t border-white/5 bg-white/[0.01]">
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div>
              <span className="inline-block px-3 py-1 rounded-full bg-sapphire-600/15 border border-sapphire-500/20 text-sapphire-400 text-xs font-semibold uppercase tracking-widest mb-4">Pourquoi Safir VTC ?</span>
              <h2 className="text-3xl lg:text-4xl font-bold mb-6">L'excellence comme standard</h2>
              <p className="text-noir-400 mb-8 leading-relaxed">Safir VTC, c'est la promesse d'un service VTC premium dans le Var et sur la Côte d'Azur. À bord d'une Tesla Model 3 noire 2022, profitez d'un trajet silencieux, confortable et éco-responsable avec un chauffeur professionnel dédié.</p>
            ...
            <div className="relative">
              <div className="aspect-[4/3] rounded-2xl overflow-hidden">
                <img src="https://images.pexels.com/photos/3802510/pexels-photo-3802510.jpeg" alt="Chauffeur Safir VTC" className="w-full h-full object-cover opacity-80" />
      ...
      <footer className="border-t border-white/5 bg-noir-950">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12">
          ...
          <div className="border-t border-white/5 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-xs text-noir-600">© 2025 Safir VTC. Tous droits réservés.</p>
