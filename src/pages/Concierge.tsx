...
import { mockConciergeOffers } from '../lib/mockData';
import type { ConciergeOffer } from '../lib/supabase';

interface ConciergeOfferWithId extends ConciergeOffer {
  id: string;
}
...
export default function ConciergePage() {
  const [offers, setOffers] = useState<ConciergeOfferWithId[]>(
    mockConciergeOffers.map((offer, idx) => ({ ...offer, id: `offer-${idx}` }))
  );
  ...
  const filteredOffers =
    selectedFilter === 'all'
      ? offers
      : offers.filter((o) => o.type === selectedFilter);
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      setOffers(
        ...
        ) as ConciergeOfferWithId[]
      );
    } else {
      const newOffer: ConciergeOfferWithId = {
        id: `offer-${Date.now()}`,
        type: (formData.type || 'hotel') as OfferType,
        name: formData.name,
    ...
    <div className="min-h-screen bg-noir-950">
      ...
      <div className="border-b border-sapphire-900 bg-noir-900">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <h1 className="text-4xl font-bold text-white mb-2">Safir Concierge</h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-noir-800 rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-sapphire-300">
                {offers.length > 0
                  ? (
                      offers.reduce((sum, o) => sum + o.commission_percentage, 0) /
                      offers.length
                    ).toFixed(1)
                  : '0'}
                %
