...
import { mockSeoPages } from '../lib/mockData';

export default function SeoPage() {
  const [pages, setPages] = useState<SeoPageT[]>(mockSeoPages);
  ...
  const generateContent = () => {
    ...
    switch (page_type) {
      case 'city':
        slug = `vtc-${generateSlug(name)}`; title = `Chauffeur VTC à ${name} - Service Premium | Safir VTC`; metaDesc = `Service VTC premium à ${name}. Chauffeur privé, transferts aéroport, gare. Réservation instantanée.`; h1 = `Chauffeur VTC à ${name}`; content = `Safir VTC propose un service de chauffeur privé premium à ${name} et ses environs. Nos conducteurs expérimentés vous offrent un confort et une fiabilité sans égal pour tous vos trajets.`; break;
      case 'airport':
        slug = `vtc-aeroport-${generateSlug(name)}`; title = `Transfert VTC Aéroport ${name} | Safir VTC`; metaDesc = `Transfert VTC aéroport ${name}. Prise en charge, suivi de vol en temps réel.`; h1 = `Transfert VTC Aéroport de ${name}`; content = `Service de transfert premium vers l'aéroport ${name}. Safir VTC assure votre transfert avec suivi des vols en temps réel et confort premium.`; break;
      case 'station':
        slug = `vtc-gare-${generateSlug(name)}`; title = `VTC Gare ${name} - Chauffeur Privé | Safir VTC`; metaDesc = `Chauffeur VTC à la gare ${name}. Transfert gare premium.`; h1 = `Chauffeur VTC à la Gare ${name}`; content = `Transfert en VTC depuis la gare ${name}. Prise en charge directe en sortie de gare, accueil personnalisé, et confort optimal.`; break;
      case 'transfer':
        slug = `transfert-${generateSlug(name)}`; title = `Transfert VTC ${name} | Safir VTC`; metaDesc = `Transfert VTC ${name} en voiture premium.`; h1 = `Transfert VTC ${name}`; content = `Transfert en VTC pour ${name}. Safir VTC offre un service de chauffeur privé rapide et fiable pour cette liaison.`; break;
    }
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      setPages(pages.map((p) => p.id === editingId ? { ...p, ...formData as Partial<SeoPageT>, updated_at: new Date().toISOString() } : p));
    } else {
      const newPage: SeoPageT = { id: `s${Date.now()}`, slug: formData.slug || '', page_type: formData.page_type || 'city', title: formData.title || '', meta_description: formData.meta_description || '', h1: formData.h1 || '', content: formData.content || '', faq: formData.faq || [], is_published: formData.is_published || false, created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
  ...
  const handleDelete = (id: string) => { if (confirm('Supprimer cette page ?')) setPages(pages.filter((p) => p.id !== id)); };
  const togglePublish = (id: string) => { setPages(pages.map((p) => p.id === id ? { ...p, is_published: !p.is_published, updated_at: new Date().toISOString() } : p)); };
