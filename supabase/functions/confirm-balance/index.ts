...
serve(async (req) => {
  ...
  try {
    ...
    // Only confirm a reservation whose deposit is already paid — avoids
    // re-processing an already-confirmed booking if the client retries.
    const updateRes = await fetch(
      `${SUPABASE_URL}/rest/v1/reservations?id=eq.${reservationId}&status=eq.deposit_paid`,
      {
        method: 'PATCH',
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/payments`, {
      ...
      body: JSON.stringify({
        ...
        amount: amount ?? (reservation.total_price - reservation.deposit_amount),
        method: 'card',
        type: 'balance',
        status: 'completed',
      }),
